...
### 供应链事件恢复(1.0.74 事件 · 随本版一并交付)

- 恶意 npm-only `1.0.74`(带 install-time 载荷、无对应 Git tag/GitHub Release)已撤下,`SECURITY.md` 公布其哈希与处置边界;`1.0.74` 与未发布成功的 `1.0.75` 版本身份一并跳过,本 `1.1.0` 是事件后首个真正发布的可信版本。
- npm 发布改为 `npm-production` 保护环境中的一次性细粒度 token(只进最终发布步骤、占位符使用、发布后即撤)+ GitHub OIDC provenance;九个 tarball 发布前过精确文件白名单与恶意载荷回归,依赖包先发、主包后发。仓库残留旧 `NPM_TOKEN` 已撤销。
- npm 更新器只信任官方 registry,核验包结构、精确平台版本、SHA-512、registry 签名与 provenance;缺证明或带生命周期脚本的版本一律拒绝。安装文档与官网复制命令固定官方 registry,避免仍缓存 `1.0.74` 的第三方镜像继续下发。
- 续跑、批准执行与自然语言"继续"统一恢复同一份未完成计划与游标;意图超时不再把明确写入降级成只读,评审服务故障改为持久化暂停+有界熔断,不再重开计划或修复无关源码。
