...
- 底座子进程不再继承 UmaDev 自己的发布/CI 凭据:每个底座 CLI(claude/codex/opencode/grok/kimi)及其孙进程从通用 spawn 扼制点出去前,`NPM_TOKEN`/`NODE_AUTH_TOKEN`/`GITHUB_TOKEN`/签名证书/`UMADEV_*` 会被擦除——正是被盗凭据蠕虫的同类暴露面;底座自身的 provider 认证(`ANTHROPIC_API_KEY` 等)和治理信号完整保留,不影响登录。
- opencode 环回控制服务器的密码改由操作系统 CSPRNG 生成(此前基于时间+pid+栈地址,熵低且部分可预测,同主机进程可暴力猜解)。
- 官网依赖 `nanoid` 的 HIGH 级漏洞(GHSA-2v37-7h3g-55p8)修复,`npm audit` 归零。

### 修复(交互 / 结算 / 资源)

- 后台子代理的终止信号丢失时,主线输出不再被无限缓冲——此前会让主 agent “哑掉”并被误判为空闲挂死;现加封顶,超限即释放并停止缓冲(fail-open)。
- 子代理分组缓冲加数量封顶,一个丢失终止信号的会话不再无界增长。
- claude 的问答/审批应答补齐“已应答”幂等门:对已被撤回或已应答的请求不再重复写回底座。
- 无界面 Guarded 下,底座“扩展权限(扩沙箱)”的请求不再被静默自动批准——改为按档升级(Guarded/Plan 需确认、Auto 保持自主)。
- 运行中排队的 steer 指令在锁中毒时不再被静默丢弃(与全仓库的中毒恢复约定一致)。

### 稳定性

- 原生 PTY/ConPTY 终端契约测试的首帧与交互超时余量放宽,根治其在满载 Windows CI 上的冷启动时序 flake(纯余量,不改验证内容)。

### 验证

- 五条独立审查线对全库真实代码深审;Agent、TUI、Host 全套测试通过,严格 Clippy、格式、严格 rustdoc、i18n 三语一致性与工作区架构门通过;Rust 与官网依赖审计干净,发布链路继续强制拒绝任何声明 `scripts` 的 npm 包(把 install-time 攻击变成硬拒绝)。

## [1.0.75] - 2026-08-08

供应链事件恢复版 · 续跑与评审状态机最终闭环 · npm 可验证发布

### 修复

- 续跑、批准执行和自然语言“继续”统一恢复同一份未完成计划与游标；意图识别超时不再把明确写入请求降级成只读解释，也不会因评审基础设施暂时不可用而重开计划、重复旧结论或修复无关源码。
...
- npm 正式发布使用 `npm-production` 保护环境中的一次性细粒度 token 认证，并保留 GitHub OIDC 生成的 provenance；token 只进入最终发布步骤，通过临时 npmrc 的环境变量占位符使用，发布后立即从 GitHub 删除并在 npm 撤销。发布前对九个 tarball 执行精确文件白名单和恶意载荷回归，依赖包先发布、主包最后发布，避免半发布版本成为用户入口。
...
- GitHub 仓库中残留的旧 `NPM_TOKEN` secret 已撤销；恢复发布只接受重新签发、限时且限定九包写权限的 token，并在发布验收后删除。安装文档与官网复制命令显式固定 npm 官方 registry，避免仍缓存恶意 `1.0.74` 的第三方镜像继续下发旧包。
- Unix 原生安装器在候选二进制运行期间实时执行 4 KiB 版本输出上限，并在成功、超限和超时三条路径都回收独立进程组；恶意或损坏的候选不能再留下后代进程。Grok 强杀回归改验证真实 TERM/KILL 与后代消失语义，不再把宿主暂停误判成清理失败。
- CI 与 Release 的 Windows-GNU 工具链安装增加网络重试、请求上限、锁等待上限和步骤硬超时；托管 runner 或 apt 镜像异常不再无界占住发布流水线。基于 JavaScript 的 Action 全部切换到 Node 24，复合 Action 与传递引用也固定为完整、已验证的提交 SHA。
- Hook 上下文、知识库与审计日志并发回归改用独立测试状态和测试等待预算，不再读取 runner 的全局 HOME，也不再把 runner 调度延迟误判为产品锁失败；生产权限及 2 秒/250 毫秒忙碌上限保持不变。

### 验证

- 全工作区测试、doctest、严格 Clippy `-D warnings`、格式和差异门禁通过；Agent 2,119 项、TUI 1,445 项通过（另有 1 项手动性能基准按设计忽略）、CLI 391 项、CLI E2E 25 项、Git 事故 3 项、进程 19 项及原生 PTY 回归全部通过。
- 发布/更新安全契约、终端契约、npm smoke、脚本语法及 ChainDrop 载荷拒绝回归全部通过。
...
- `/continue`、`/tasks resume`、自然语言“继续 / 继续推进执行 / 批准执行”等续跑入口统一到同一条持久化状态迁移：只恢复原计划、原游标、原证据与原需求，不再另开计划；没有可恢复工作时会明确提示，而不是凭空启动流水线。单独的“确认 / 批准”不会脱离现有计划制造新的写权限。
- 明确的修复、开发、完成、写入与 Git 提交请求在意图识别超时或临时中断时仍保留可写语义，不再降级成 `explain / fast` 只读子会话；Plan 模式继续严格只读。CLI 与 TUI 使用同一判定，避免直接使用底座可写、切到 UmaDev 却反复要求重新授权。
- 限流、临时断线、底座停止、`read-only resident` 冲突会结算当前内存回合但保留磁盘计划；再次续跑承接原任务，不再清空计划、重复批准或回到只读沟通循环。无恢复游标时在拿锁、创建会话或修改状态前失败关闭。
- 评审基础设施不可用与代码评审不通过彻底分离：首次故障暂停在同一步，连续故障打开有界熔断；显式续跑会在同一游标和证据上重试，绝不会把评审超时伪装成源码缺陷、触发无关 Docker 全测/自动修复或反复团队评审。旧评审计数的重置竞态同步修复。
- 常驻执行的“600 秒硬停”改为 1,800 秒滑动无进展上限与 3,600 秒绝对上限；有真实进展的长任务不会被误杀，无进展的工具/思考循环仍会可靠停止并保留续跑点。

### 验证

- 全工作区 `--all-features --all-targets`、doctest、严格 Clippy `-D warnings`、严格 rustdoc、格式和差异门禁全绿；Agent 2,119 项、TUI 1,443 项（1 项手动性能基准按设计忽略）、CLI 391 项、CLI E2E 25 项、Git 事故 3 项、进程 19 项及其余工作区测试全部通过。
- npm 版本锁、终端/CLI 分类契约与 release 构建通过；正式 tag 仍只会在 main 的精确提交 CI 与 Release 预验证全部通过后创建。
...
- 项目状态、审计、计划、任务账本、治理基线、知识和证明文件统一使用能力根目录、有界读取、原子写入与 no-follow 规则；拒绝中间目录/叶子符号链接、reparse point、FIFO、设备文件、硬链接替换和读取后的身份竞态，避免仓库内容把 UmaDev 引出项目边界。
- JSONL / 快照 / 索引损坏时不再返回截断的半真相或覆盖原始证据；并发读改写使用有界跨进程锁，目录洪泛、超大条目和未知历史事件都会明确降级或失败关闭。
- 踩坑、经验、项目事实和向量召回保持来源、时间与验证状态；同一代记录的有效性判断修复 TOCTOU，缺失状态不再误判为成功。BM25、repo-map、向量语料和本地 embedding 的文件数、总字节、单项大小、批量与并发均设硬上限。
- 所有写入持久化前统一脱敏命令、日志、工具结果和证明包中的密钥样式数据，避免把底座输出里的凭证复制进 `.umadev`、审计或发布证据。

### 修复(底座与跨平台交互)

- Claude Code、Codex、OpenCode、Grok Build 与 Kimi Code 的 CLI 版本均只作诊断，不作启动白名单；标准能力来自实时协议。Grok 的私有 xAI 方法按逐能力源码证据启用，未知协议代际仍使用标准 ACP；Kimi 的模型、模式和推理控制只认会话实时 `configOptions`，不会因版本号被拒绝。
- GitHub #49 根因修复：Director 恢复时只指纹当前计划明确声明的 PRD / architecture / UIUX 文件；历史 `output/*` 同类文档不再覆盖语义 key、误判当前黑板过期并重开上游步骤。多份当前文档按路径确定性聚合，旧计划没有文件身份时保守忽略历史目录。
- 复制成功提示继续停留在短暂状态区，并改按 Unicode grapheme 统计可见字符；组合字符、ZWJ emoji 和旗帜不再虚增。Overlay 会过滤 CR、ESC、BEL、NUL 等终端控制字符；宿主输入、MCP、目录信任、部署和 PR 提示补齐中简 / 中繁 / 英文且不混排。
- Windows、macOS、Linux GNU/musl 的终端、调整窗口、滚轮、可拖动滚动条、同步输出、进程树回收和安装更新路径完成回归；Windows npm 自更新仍由 JS shim 执行并对 IDE/终端占用给出精确恢复，七个平台包继续核验 OS / CPU / libc 和真实二进制版本。
...
- 权限失控根因根治:Guarded 模式下批准一次普通 shell(或一次工作区外读取)不再永久免问全项目所有 shell、沙箱扩展与计划确认——`permission-expansion` / `plan-confirmation` / `external_directory` 这些语义请求动作不再与真实 shell 共用同一个 ledger key,结构上不可记忆,每次重新询问。
- “底座只读、改不了”的根因修复:当用户处于可写档(Guarded/Auto)、而本回合被大脑判为咨询/解释因而以只读进程执行时,会显式提示“本回合按只读执行;若要改代码请说明修改意图再发一次”,不再让底座静默拒绝写入而档位显示“自动过门”。
- Grok Build 计划模式僵死根治:`/run` 与聊天路径下,底座的“计划审阅 / 退出计划模式”确认现在按其自有契约应答并可弹出真实交互面(Auto 档自动批准、只读档批准即升到可写档实施);计划审阅不再被无条件取消,底座不再卡在会话级计划模式、把后续每次执行回报成“User rejected the execution”。
- 结算不再假报:被中断、等待用户回答、或工作区零源码却声称完成的回合不再显示“[agentic] 完成”,而是如实标注“已中断 / 正在等你回答 / 判为未完成”;工具执行中途失败的回合不再同屏出现“运行中”与“已中止”互相矛盾的状态。
- 底座请求送达修复:opencode 的权限/问题帧在缺少 sessionID 时不再被整帧丢弃(Guarded 下每次写入都发权限请求,原本会让全部审批弹窗消失、回合静默卡死);kimi 的问答请求补齐协议契约,超时/取消/无界面各拒绝路径不再向底座写协议错误;claude 无法关联的控制帧不再弹出无法应答的幽灵审批;opencode 未知帧不再静默丢弃而是记录告警。
- 底座撤回请求不再杀会话:ESC 中断后误点旧审批卡片不再因回答一个已撤回的请求而终止整个 opencode 会话;死问卷会被新请求顶替而不是静默拒绝;回合结束时清理挂起问卷,死 picker 不跨回合存活。
- 交互与显示:粘贴文本进入问答 picker 会落入被读取的备注缓冲而非被忽略的隐藏输入框;通用问答条恢复标签、按键提示与敏感遮蔽说明;矮终端下 gate 选择项按选中项开窗、不再让不可见项被数字键盲选;降级/评审停摆时提示行与状态不再互相矛盾;闸门打开时排队计数不再消失。
- 旧终端可达性:新增 `/queue` 命令与队列内 `s` 立即发送(Ctrl+; / Ctrl+Enter 在多数终端无法编码);Grok 计划审阅面板显示行位置计数。
- 跨工作区读取可见化:底座访问工作区之外的文件时发出一次性可见提示(纯观测、不拦截),解决“在空目录里重构却读到相邻旧项目”的意外。
- 精确用量按精确入账:底座上报了精确用量时按精确记账(含输入/输出/缓存拆分),`/usage` 不再把精确数据标为“估算”,Grok 的精确成本不再显示为未知。

### 新增

- 底座撤回请求的撤回机制基础(`HostRequestSettled`):当底座撤回它自己发起的请求(取消、子代理结束、超时、会话/回合结束)时,按请求 id 撤回对应的问答 picker 与审批条,并给出可见提示;本地 Guarded 暂停(无底座 RPC)不受影响。

### 验证

- Agent、TUI、Host、Runtime、Knowledge、Process 全套测试通过;严格 Clippy `-D warnings`、格式、严格 rustdoc、i18n 三语一致性与工作区架构门通过。
- CI flaky 测试簇根治:并发去重测试反映真实有界锁契约、进程 argv/回收探测改为负载鲁棒的等待、进程组销毁安全测试改为全平台串行执行并从并行工作区运行中排除,恢复干净可信的 CI 信号。

## [1.0.68] - 2026-07-28

文档确认门可恢复 · 本轮治理精确归因 · Headless 与 TUI 行为一致

### 修复

- Guarded 模式的 `umadev run` 无论在 TUI 还是无界面终端都会真正停在文档确认门并持久化状态；纯文档 Fast 任务改为执行自己的文档步骤，即使文档是最后一个交付物也不会悄悄跳过确认。
- `/continue` 可以恢复“所有文档步骤已完成、只等待确认”的计划，关闭确认门后直接进入文档感知的最终验收，不重写已完成文档，也不把完整计划误判为“无可恢复任务”。Auto 模式仍保持端到端自动执行。
- Director 在新运行写入任何内容前记录有界源码治理基线；最终 QC 只把本轮新增或实际改动文件中的违规归因给本轮，旧项目里内容未变化的既有问题会被明确忽略，不再让文档任务扫描整个棕地代码库、反复修复无关历史债务。
- 重新验证意图超时兜底和 Codex 会话回收：无法完成模型意图判断时，“查找本地仓库”仍进入可使用只读工具的解释路径；Codex app-server 结束时继续按进程组 / Windows Job Object 回收完整进程树。

### 验证

- Agent 2,037 项核心测试在单线程模式全部通过；工作区架构门、CLI 379 项及集成测试、Codex 进程树回收、意图兜底、格式检查和全工作区严格 Clippy `-D warnings` 通过。GitHub #47 已确认为撤回误建，#48 的复现链已由新增回归覆盖。

## [1.0.67] - 2026-07-28

当前请求优先 · 证据与记忆可追溯 · 底座能力化适配 · 启动与帮助交互收敛

### 修复

- 当前用户请求成为本回合唯一授权来源；历史对话、项目文档、记忆、工具输出和子 Agent 结果只作为带来源的证据。普通聊天不再携带旧计划、构建规范或项目记忆，解释类请求只读取必要项目轮廓，明确修改与提交请求不会再被陈旧的 `read-only` 状态压住。
- 项目事实和未决事项改为按当前需求召回：事实必须来自用户明确陈述、仓库文件核验或文件系统核验，旧版推断记录和无来源记录留在审计区但不再进入底座提示词；普通请求不会看到无关偏好，显式盘点仍可查看全部。
- 项目指令、仓库轮廓、项目事实和历史运行笔记统一使用有界、类型化、无权限的引用封装；正文中的伪造标签、提示词注入或 `Request:` 文本只能作为数据，不会改变当前任务、授权、工具边界或评审结论。
- 启动页压缩为结果优先的两段引导，80×24 终端首屏不再溢出；`/help` 支持窄终端按显示宽度换行和完整滚动，命令签名不再被截断。启动过程也不再静默修改 Claude / Kimi 的项目 hook，治理集成只由显式 `umadev install --base ...` 安装。

### 架构与治理

- 底座差异改由实时会话能力声明驱动，Agent 核心不再按底座名称分支推断实时治理能力；Claude Code、Codex、OpenCode、Grok Build 与 Kimi Code 都继续接受用户已安装的官方版本，审计基线只用于协议漂移检测，不作为运行时版本白名单。
- 注释治理阈值与实现统一：只治理新增或恶化的长普通说明块和注释显著多于代码的改动，不要求为了过门补注释，也不把文档、许可证、生成文件和测试计入普通注释债务。
- 评审基础设施不可用仍按运行故障有界结算，不会改写源码、重跑无关测试或把超时伪造成代码缺陷；用户可从 `/plan` 查看完整计划、团队、评审与排队状态。

### 验证

- 全工作区 `--all-targets` 测试通过，严格 Clippy `-D warnings`、格式与差异检查通过；官网 lint 与生产构建、npm 启动器/更新器/平台包契约和版本锁通过。原生 PTY 已验证 80×24 启动、`/help`、`/plan` 与退出路径。

## [1.0.66] - 2026-07-27

完整计划可见 · 评审不可用有界收口 · 跨平台进程与文件系统加固 · 发布渠道统一

### 修复

- `/plan` 现在同时展示 UmaDev Director 计划、底座原生计划、完整角色团队、阻断项、修复建议与 advisory；即使只有底座计划也不再误报“暂无计划”。窄终端按真实显示列裁切中文与组合字符，44×12 的计划、团队和评审内容可从 transcript 顶部到底部完整到达。
- 必需评审的基础设施故障与语义不通过彻底分离：正常完成但格式错误的回复只在同一只读席位修复一次 schema，空回复、失败、工具调用和超时绝不被“挽救”为通过；随后不可用席位只在新只读会话重试一次，所有串行 fork 共用默认 60 秒总预算。Auto 在内部重试耗尽后当场持久化终止，Guarded 只允许一次 `/continue`；两种模式都不再把超时送去修改源码、反复跑 Docker、伪造通过或无限等待。
- CLI 与 TUI 会在底座探测、登录、会话创建和任务账本之前识别已终止的评审凭据；`/cancel`、新 `/run` 和单发/持续两条路径都会先结算旧暂停，既不复活过期任务，也不制造空任务账本。即使历史状态记录了已卸载底座，终止结果仍立即、可解释地返回。
- 当前用户明确要求修复、登记、保存、提交或写文件时，可写授权在本回合生效；查询、解释和 Plan 仍保持只读。底座模型不能再用旧会话的 `read-only` 覆盖当前明确写入意图，也不能因状态查询触发团队评审或旧计划续跑。
- 修复输入、复制和渲染边界：多行粘贴不再因终端字节解码变乱码，复制成功提示进入短暂状态区而不是占据消息流，Grep 的匹配统计采用有界真实计数，不再显示远大于项目字符数的异常大整数。

### 安全与可靠性

- 工作区、源码、配置、审计、知识、证明包和治理扫描统一采用有界读取、全局条目/字节预算与 no-follow 规则；超限、符号链接、FIFO、设备文件和目录洪泛会显式降级或失败关闭，不再返回截断的“半真相”、越出项目根或阻塞 TUI。
- 团队评审材料现在以本轮真实变更集和计划声明文件为强制范围，需求/前后端/测试路径仅用于补充抽样；清单会明确披露发现、抽样、遗漏和敏感文件排除。强制文件未完整进入材料、变更集不可读取或目录扫描不完整时，评审只能报告 unavailable，不能再把前缀或漏文件冒充完整覆盖。
- 外部命令、验证、部署与预览服务统一使用有界 stdout/stderr 尾部、单一绝对超时和进程树回收；流式调用超时不会再用完整预算冷启动第二次调用。父进程退出但子进程仍持有管道、无换行洪泛、端口重用与 PID 重用均有回归，Linux/macOS 进程组和 Windows Job Object 不再留下僵尸或让运行永久等待。
- Grok Build 与 Kimi Code 继续按官方源码契约和实时协议能力适配，不锁死用户 CLI 版本；认证、权限、恢复、模型/模式配置和 ACP 身份失败均以明确边界结束。`mcp install --backend all` 改为事务式更新，任一底座失败会回滚已写配置。
...
宿主 Git 事务闭环 · 写入权限与评审暂停收敛 · 队列、恢复和取消安全

### 修复

- 明确的 Git 提交请求由 Rust 宿主在进入模型和通用构建流水线前接管：Plan 保持零写入，Guarded 最多只确认一次，Auto 只执行一次；事务使用固定参数、精确暂存集和原子回滚，不再反复要求“确定提交”，提交成功后也不会误触发团队评审、Docker、自动修复或完整交付流程。
- 当前回合明确要求修复、登记、保存、写入或创建时，权限由用户指令与信任档位共同确定，不再由底座模型把可写请求留在 `read-only`；明确只分析和 Plan 模式仍是不可越权的只读上限。
- 必需评审超时、启动失败或返回不可解析内容时进入类型化、可恢复的运行时暂停，不伪造通过，也不当成源码缺陷交给写者修复；`/continue` 只重试被暂停的评审边界，源码指纹未变时不重跑已完成的源码工作、确定性检查或无关评审。
- `/plan` 与底部排队计数现在读取同一份宿主 FIFO 状态；转向消息、后续任务和宿主 Git 事务不会再出现“显示已排队但计划为空”或被当前模型回合重新解释的分叉。
- 底座原生命令与 UmaDev 流水线指令进一步隔离；原生命令、Git 事务及其恢复路径不会消费陈旧需求、旧计划或上一轮授权，也不会被 `/continue`、redo、revise 和常驻会话恢复重新送进通用模型路径。
- 取消、恢复与并发完成采用明确的宿主终态：已发布结果优先，取消、异常退出和正常完成分别结算；迟到事件不能清除新一代会话状态，暂停的 gate、预算、队列和底座 session hand-back 都会被保留或有界失效。

### 工程治理

- TUI 的路由决策、宿主 Git、运行暂停和 Director 驱动进一步拆分为独立模块，并为控制器体积、依赖方向和执行后置条件增加架构回归门，减少单文件继续膨胀和模型文字反向驱动宿主状态的风险。
- 新增 Git 事务黑盒、Plan / Guarded / Auto 权限、队列与重放防火墙、评审暂停、取消竞态和 CLI/TUI 入口回归；本地版本锁、格式、静态检查与相关测试用于发布前验证，远端三平台 CI 和七个原生二进制发布门仍在创建正式标签后最终放行。
...
- 明确的“修复、登记、记录、保存、写入、创建”等可写请求在 Guarded / Auto 下获得可写会话，不再被底座意图判断误留在 `read-only`；用户明确要求只分析以及 Plan 模式仍保持只读硬边界。
- `/run` 和普通输入在当前底座未安装、不可启动或探测明确失败时会在规划与状态写入前停止并保留需求；切换底座不会再先提示成功、随后才因 PATH 缺失而中止。
- 进度、状态和历史查询保持观察型路径；存在确定性阻塞时先修根因，不再同时召集 critic 产生重复超时、无关评审和“评审 → 自动修复 → 再评审”循环。
- 每个新运行都会建立独立工作区基线；scope 扫描排除 `.DS_Store`、缓存、覆盖率、本地密钥与底座私有配置，避免按 mtime 或旧运行基线把未改文件和本地环境误报成本轮修改。
- Grok Build 工作区初始化超时会明确指向目录权限 / 信任、登录与 CLI 版本预检；Codex 高危沙箱提示明确区分 UmaDev 启动的底座会话和用户单独打开的 Codex 窗口。
- Claude Code、Codex、OpenCode、Grok Build 与 Kimi Code 均不使用 CLI 版本白名单：版本号只作诊断，能力由实时协议、身份和运行时安全规则决定。Grok / Kimi 的源码契约已同步到最新审计提交，并通过源码漂移测试持续跟踪。

### 验证

- Agent 1,835 项与 TUI 1,210 项单元测试通过，Host 589 项测试通过；本次修改包的严格 Clippy、`cargo fmt`、差异检查、Grok / Kimi 源码漂移测试、版本锁和官网生产构建通过。发布仍由 GitHub 三平台质量门、七个原生二进制构建和发布后 registry 完整性校验最终放行。
...
- 用户在当前回合明确要求“修复 / 修改 / 实现”时，该指令会覆盖底座模型误判的 `read_only` 或缺失授权，直接选择可写会话；只有用户明确要求只分析，或主动进入 Plan 模式时才保持只读。
- Claude Code 的项目级治理 hook 改为合并写入本地 `settings.local.json`，不再覆盖用户已有 hook；写入采用原子替换并拒绝符号链接，`doctor` 会逐阶段核验实际生效的 UmaDev 二进制。
- 踩坑记录补齐时间、独立 episode、修复生命周期与精确验证归因；`/lessons` 只呈现真正复发后形成的待验证或已验证经验，跨项目晋升继续执行隐私脱敏和来源约束。记忆维护增加总时限、单次时限与次数上限，失败不会拖住用户回合。
- 计划状态、知识索引与项目事实的持久化进一步原子化、限大小并拒绝路径逃逸/符号链接；中断、队列、恢复与评审路径均采用有界退出，避免旧状态复活或无效循环。
- 发布链在打包前校验七个原生二进制的 OS、CPU 与 libc 文件头，拒绝把错误架构的可执行文件发进 npm；registry 可见性、完整性与 `latest` 提升均有可重试且不可回退的校验。
- 治理分词器把 Unicode 字符偏移一次性转换为 UTF-8 字节偏移，消除大文件、多注释场景下的平方级重复扫描；当前工作树 259 个源文件的优化版完整扫描实测约 1.4 秒、0 条命中。
- npm 的 ES5 版本门启动器使用严格的文件头治理声明，既保留旧 Node 的友好升级提示，也不再被通用 `var` 规则误报；放在文件头之外的声明仍会被拒绝。
