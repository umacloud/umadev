...
- 必需评审的基础设施故障与语义不通过彻底分离：不可用席位只在新只读会话自动重试一次，语义 pass/fail 绝不重试；所有串行 fork 建立共用默认 60 秒总预算，未尝试席位明确记为 unavailable。相同评审边界再次失败后打开持久熔断并真实结束本轮，不再把超时送去修改源码、反复跑 Docker、伪造通过或无限要求 `/continue`。
