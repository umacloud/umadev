...
//
// umadev — modern JS launcher (the real logic). It is NOT the process entry
// point: `bin/cli.js` is a tiny ES5-only version gate that require()s THIS file
// only after confirming Node is new enough, so an ancient-Node user gets a clear
...
// The ONLY verbs that actually retrieve knowledge and therefore need the ~224MB
// local embedding model fetched first. This is an explicit ALLOW-set, not a deny
// list: every other verb — read-only ones (history/usage/lessons/report/spec),
// emergency ones (rollback/verify/deploy), and the utility verbs
// (update/doctor/init/pr/…) — must start INSTANTLY and never block on a download
// it will not use. Keeping it as an allow-set means a NEW verb defaults to
// no-download; the previous deny list did the opposite, so `umadev rollback <ts>`
// on a fresh install awaited a 224MB fetch it never touched. The model is still
// fetched lazily, only on these retrieval verbs (and the bare interactive TUI).
const NEEDS_MODEL = new Set(['run', 'quick', 'redo', 'continue', 'revise']);

// Decide whether THIS invocation will retrieve knowledge (and so needs the local
// embedding model fetched before launch). Pure + exported so the classification is
// unit-tested without a network or a real launch. Bare `umadev` (no verb) is the
// interactive TUI, which retrieves; every explicit non-retrieval verb/flag returns
// false and launches at once.
function invocationNeedsModel(argv) {
  const first = argv[2] || '';
...
// Block this thread for ~`ms` without spinning the CPU. `Atomics.wait` on a throwaway
// SharedArrayBuffer is the only dependency-free synchronous sleep in Node; if it is
// unavailable (locked-down runtime) it falls back to a short bounded wait. Used to
// pace the stdin EAGAIN retry below instead of a tight busy-loop that pins a core.
function sleepSync(ms) {
  try {
    ...
    while (Date.now() < end) {} // Atomics unavailable — brief bounded wait
  }
}
...
function readLineSync() {
  ...
  for (;;) {
    ...
    try {
      ...
      // TTY not ready yet: wait a beat instead of a tight EAGAIN busy-loop that
      // would pin a CPU core while the user is still deciding at the prompt.
...
async function main() {
  ...
  // interactive upgrade prompt, so let them fall through to the binary's own
  // `update` help instead of stranding the user at a `[y/N]` question.
