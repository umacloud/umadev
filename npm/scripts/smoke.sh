...
INSTALLED_MANIFEST="$(trusted_manifest "$INSTALLED_VERSION")"
...
make_stub() {
  STUB_MANIFEST="$(trusted_manifest "$2")"
  cat > "$UPD_TMP/bin/$1" <<STUB
#!/bin/sh
case "\$1" in
  --version) echo "9.9.9"; exit 0 ;;
  view)      echo '$STUB_MANIFEST'; exit 0 ;;
esac
...
case "\$1" in
  --version) echo "9.9.9"; exit 0 ;;
  view)      echo '$INSTALLED_MANIFEST'; exit 0 ;;
esac
echo "npm-called: \$*"
STUB
