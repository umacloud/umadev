...
**你不是雇了一个总监，你是雇了一整支开发团队。** 团队里有一个**协调者**（技术负责人，即总监席位）——它不亲自敲代码，只做三件事：路由意图、调度团队、把关交付。底座是干活的工程师（思考、调研、设计、写代码、评审都是底座的认知）；协调者是确定性工具层（拆可视计划、设置确认门、实时治理、跑质量门、留下审计证据）。独立开发者、小团队，瞬间拥有一支完整、专业、有工程纪律的团队。

团队的八个角色各有自己的产物，而非写死的判断逻辑：
...
- **协调者（总监席位）** — 路由意图、拆计划、调度团队、每道门汇总裁决、留证据
...
当你说出一句需求，umadev 不是直接把它丢给底座，而是像一支训练有素的团队（由协调者统筹）那样先想清楚、把计划摆给你看、带上团队规范和对你代码库的理解去干、逐步调度团队并每步验收、最后留下交付证明。一次输入会流经这几层（每一步向底座的咨询都 fail-open——失败就退回确定性兜底，绝不卡死）：
...
| 协调者（总监席位） | 路由意图、持有计划、调度团队、把关每道门、留审计证据 |
...
团队带你交付听起来像把判断权交给模型——其实恰恰相反。umadev 的可信来自把"模型说了什么"和"硬信号是什么"严格分开：
...
**9 阶段主链不是每个需求都走的固定漏斗，而是协调者为"完整商业级 greenfield 交付"选择的最完整的交付路径。** 底座的模型先判断意图（上文"umadev 如何工作"①）：对话不进流程、小修走快路径、bugfix 不组队；只有一个完整产品需求，协调者才把计划展开成下面这条主链。规范里这条链就是 `standard` profile 的全程（见 [spec §4.1 / §9.5](spec/UMADEV_HOST_SPEC_V1.md)）——是**怎么交付一个完整产品**的契约，不是"每句话都得走九步"。
