...
export const releases = {
  zh: [
        { ver: "1.0.32", date: "2026-07-06", current: true, title: "研究驱动的开发团队交互强化 · typed 交接契约全线打通 · 门禁显示一致性", changes: [["新增", "座位能力卡(Seat Card):每个座位(PM/架构/设计/前端/后端/QA/安全/DevOps)有一张 typed 自描述能力卡,声明它 owns 什么、reads 哪些契约输入、produces 哪些产出;roster 自描述,每次交接都成为显式可校验的契约"], ["新增", "每跳交接校验(双向):座位运行前即比对声明的输入/产出与黑板现状 —— 缺声明输入(错位)或没产出其负责的 artifact(规格缺口)都在交接处折进 verdict 暴露,不再变成下游谜团,活在评审流里"], ["新增", "两层 artifact 物化:把散文文档里的决策抽成 typed 契约(数据模型/设计令牌/验收标准)emit 到 .umadev/contracts/,与 API 契约并列,接进文档阶段"], ["新增", "verdict 结构化溯源:评审结论可带来源座位 + 关联 artifact + 诊断说明,由确定性标注器填充(不采信底座自述),打回更可诊断、审计链可重建;serde 向后兼容"], ["新增", "artifact 版本化 → DAG 陈旧失效:黑板文档带内容版本;跨会话改了 PRD/架构/UIUX 后 resume,消费该文档的步骤及下游自动翻回待办、对着变更后的上游重新推导,不信任被静默污染的旧结果"], ["新增", "黑板 public/private 私有区:两座位化解冲突时可在 .umadev/scratch/ 私有区来回,不污染公共 output/ 黑板,run 结束回收"], ["修复", "守护模式门禁文案一致:预检卡门禁行随信任模式(auto/guarded/plan)动态生成,不再底部写「逐门审核」正文却写「默认自动通过」;去掉意图卡重复的「完整构建」行"]] },
        { ver: "1.0.31", date: "2026-07-05", current: false, title: "闪屏根治 · 计数 / 滚动 / 去重修复 · 跨平台 CI 转绿", changes: [["修复", "mac 长会话每秒闪屏根治:1.0.30 为 Windows conhost 防漂移加的 ~1 秒周期性全屏重绘,在不支持「同步输出」的 Terminal.app 上就是每秒闪一下 —— 现把这个心跳限定到 Windows,mac / Linux 靠增量重绘 + 事件驱动自愈(想彻底无闪无漂移可用 iTerm2 / WezTerm / kitty / Ghostty 等支持同步输出的终端)"], ["修复", "自学习踩坑计数不再卡住:复现索引此前用原始签名建 key、却用归一化签名查找,归一化逻辑加入前存下的老记录永远匹配不上、频次冻结(反馈的「已踩 17 次」不再增长);改为索引也归一化 + 同签名保留最高计数条"], ["修复", "长转录滚动锚定不再漂移:转录超过 8000 行渲染上限触发 front-trim 后总行数被封顶,上滚锚定静默失效、阅读位置被新内容顶走;补上 front-trim 增量补偿,长流式下上滚看历史位置稳住"], ["修复", "同一 diff 不再显示两遍:底座可能在文本叙述与结构化工具调用里各带一次同一处修改(或 opencode 工具分片两个 id);加防御护栏,连续出现的完全相同 diff 折叠为一张(不同修改仍各自成卡)"], ["修复", "跨平台 CI 转绿:doctor claude-hook 测试接受「已注册但命令不解析」;TUI meta 行测试锁英文 + 加宽;终端模式 / 路径 / fork 计时等平台假设测试按平台门控;修 Windows 构建 unused-import / dead-code(-D warnings)"], ["新增", "鼠标选中 → 剪贴板复制正式生效:拖选文字经 OSC52 复制到系统剪贴板(含 tmux 透传)+ conhost 控制台守卫,随本版发布(此前已实现但未随 1.0.30 上线)"]] },
...
{ ver: "1.0.14", date: "2026-06-28", current: false, title: "Ctrl+C 不再误退 · /continue 跨会话续接", changes: [["修复", "Ctrl+C 不再退出 UmaDev:它是复制的通用肌肉记忆,误按一下就退出太容易丢会话。现在 Ctrl+C 只清空半截输入 / 提示用 /quit,绝不退出;要退出用 /quit、/exit、Ctrl+D 或双击 Esc"], ["新增", "/continue 跨会话续接:之前 /run 跑过、关掉 TUI 再打开,用 /continue 只会提示重启。现在它会自动加载持久化的 plan.json,重新贴出计划(已完成的步骤打勾)、只跑剩余步骤续上,不用重头再来"]] },
  ...
  en: [
        { ver: "1.0.32", date: "2026-07-06", current: true, title: "Research-backed dev-team interaction hardening · typed hand-off contracts wired end-to-end · gate-display consistency", changes: [["Added", "Seat Cards: every seat (PM/architect/designer/frontend/backend/QA/security/DevOps) carries a typed self-describing capability card declaring what it owns, which contract inputs it reads, and what it produces — the roster is self-describing and every hand-off becomes an explicit, checkable contract"], ["Added", "Per-hop hand-off check (both directions): before a seat runs, its declared inputs/outputs are checked against the blackboard — a missing declared input (misalignment) or an un-materialized owned artifact (a spec gap) surfaces in the verdict at the hop, not as a downstream mystery; live in the review flow"], ["Added", "Two-layer artifact materialization: the decisions in the prose docs (data model / design tokens / acceptance criteria) are parsed into typed contracts emitted to .umadev/contracts/, alongside the API contract, wired into the docs stage"], ["Added", "Structured verdict provenance: a verdict can carry its originating seat + related artifact + diagnosed note, populated deterministically (never trusted from the base) for diagnosed rework + a reconstructable audit trail; serde-backward-compatible"], ["Added", "Artifact versioning → DAG staleness invalidation: blackboard docs carry a content version; edit a PRD/architecture/UIUX doc between sessions and on resume the consuming plan steps + their downstream re-open to re-derive against the changed upstream, instead of trusting a silently-poisoned result"], ["Added", "Private blackboard scratch lane: two seats resolving a conflict work in a private .umadev/scratch/ lane without polluting the public output/ board, GC'd on run end"], ["Fixed", "Gate-display consistency: the preflight gate line is generated from the active trust mode (auto/guarded/plan), so it no longer reads per-gate approval at the bottom while the body says auto-approved; removed the duplicated full-build line on the intent card"]] },
        { ver: "1.0.31", date: "2026-07-05", current: false, title: "Flicker rooted out · counter / scroll / dedup fixes · cross-platform CI green", changes: [["Fixed", "macOS long-session flicker rooted out: 1.0.30's ~1s periodic full-screen repaint (a conhost anti-drift heartbeat) flashed once a second on Terminal.app, which lacks synchronized output. It is now Windows-only; macOS / Linux reconcile via the incremental diff + event-driven heals. For zero flicker AND zero drift use a synchronized-output terminal (iTerm2 / WezTerm / kitty / Ghostty)."], ["Fixed", "Self-learning pitfall counter no longer freezes: the recurrence index was keyed by the raw stored signature but looked up by the normalized one, so pre-normalization records never matched and froze (the '17 times' that never grew). The index now normalizes both sides and keeps the highest-count record per signature."], ["Fixed", "Scroll anchor holds on a long transcript: past the 8000-row render cap the front-trim pins the total row count, so the scroll-up anchor silently stopped holding and the read position drifted. Added the front-trim delta so it stays put while scrolled up during a long stream."], ["Fixed", "An identical diff card no longer renders twice: a base can surface the same edit both in narration and as the tool call (or an opencode tool part under two ids). A guard collapses a diff identical to the previous one; distinct edits still render."], ["Fixed", "Cross-platform CI green: the doctor claude-hook test accepts a registered-but-unresolved verdict; the TUI meta-row test pins English + widens; terminal-mode / path / fork-timing platform-assumption tests are platform-gated; fixed the Windows build unused-import / dead-code under -D warnings."], ["Added", "Mouse-select -> clipboard copy now ships: drag-select copies to the system clipboard via OSC52 (with tmux passthrough), plus the conhost console-mode guard (both were implemented but did not ship with 1.0.30)."]] },
